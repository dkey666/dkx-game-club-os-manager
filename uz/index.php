    ...
    @import url('https://fonts.googleapis.com/css2?family=Exo+2:wght@300;400;500;600;700;800;900&display=swap');
        * {
        ...
        .preloader {
            ...
            width: 100%;
            height: 100%;
            background: linear-gradient(135deg, #0a0a14, #0f0f23, #1a1a2e);
            z-index: 99999;
            display: flex;
            align-items: center;
            justify-content: center;
...
<body>
    ...
    <div class="preloader" id="preloader">
        <div class="loading-spinner">
            <div class="loading-text"><?= htmlspecialchars(APP_SHORT_NAME, ENT_QUOTES, 'UTF-8') ?></div>
        </div>
    </div>
    ...
    <button class="profile-button" onclick="openProfile()">
        <span id="profileRankDisplay">Профил</span>
    </button>
    ...
    <div class="halls-section">
        <h2 class="section-title">БИЗНИНГ ЗАЛЛАРИМИЗ</h2>
        <div class="hall-card">
            <div class="hall-title">🎯 ЗАЛ 1 - ОНЛАЙН</div>
            <div class="hall-description">Онлайн ўйинлар учун 10 та кучли ПК: CS2, Dota 2 ва бошқалар</div>
        </div>
        <div class="hall-card">
            <div class="hall-title">⚔️ ЗАЛ 2 - ОФФЛАЙН</div>
            <div class="hall-description">Оффлайн ўйинлар ва классик шутерлар учун 10 та ПК</div>
        </div>
        <div class="hall-card">
            <div class="hall-title">🎩 ЗАЛ 3 - ОНЛАЙН+ОФФЛАЙН</div>
            <div class="hall-description">Онлайн ўйинлар учун 5 та ва оффлайн ўйинлар ҳамда классик шутерлар учун 5 та ПК</div>
        </div>
    </div>

    <!-- Prices Section -->
    <div class="prices-section">
        <h2 class="section-title">💰 ТАРИФЛАР</h2>
        <div class="price-card">
            <div class="price-time">☀️ Кундузги (09:00 - 19:00)</div>
            <div class="price-amount">12 000 сум/соат</div>
        </div>
        <div class="price-card">
            <div class="price-time">🌙 Кечки (19:00 - 09:00)</div>
            <div class="price-amount">14 000 сум/соат</div>
        </div>
        <div class="price-card">
            <div class="price-time">🌃 Тунги пакет (00:00 - 07:00)</div>
    ...
    <div class="leaderboard-section">
        <div class="leaderboard">
            <div class="leaderboard-title">🏆 ТОП 10</div>
            <div id="leaderboardList">
                <div style="text-align: center; color: rgba(255,255,255,0.5); padding: 20px;">⏳ Юкланмокда...</div>
            </div>
    ...
    <script>
        ...
        async function loadUserData() {
            try {
                ...
                if (user && user.points_registered) {
                    // Обновляем баланс
        ...
        // В конце скрипта, замените инициализацию на:
        window.addEventListener('load', function() {
            hidePreloader();
            // Принудительно обновляем ранг после загрузки страницы
            setTimeout(forceUpdateRank, 1000);
        });
        ...
        async function loadLeaderboard(offset = 0) {
            try {
                ...
                if (list) {
                    list.innerHTML = '<div style="text-align: center; color: #e74c3c;">Юклашда хатолик</div>';
                }
            }
        }
        ...
        function renderLeaderboard() {
            ...
            if (!leaderboardData || leaderboardData.length === 0) {
                list.innerHTML = '<div style="text-align: center; color: #a0a0a0; padding: 20px;">Маълумот хозирча йук</div>';
                return;
            }
        ...
        function renderUserNotifications() {
            ...
            if (userNotifications.length === 0) {
                container.innerHTML = '<p style="text-align: center; color: #666; padding: 20px;">Хабарлар йук</p>';
                return;
            }
        ...
        function showBalanceIncreaseNotification(amount) {
            addUserNotification('balance_increase', `${amount.toLocaleString()} сум кушилди!`);
        ...
        function hidePreloader() {
            ...
            if (preloader) {
                preloader.classList.add('exploding');
                setTimeout(() => {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            setTimeout(() => {
                ...
                if (isFirstTime) {
                    addUserNotification('welcome', 'Хуш келибсиз!');
                }
            }, 3000);
...
<div id="notificationsModal" style="display: none; position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.9); z-index: 10001; overflow-y: auto; padding: 20px;">
    <div style="max-width: 600px; margin: 80px auto 40px; background: linear-gradient(135deg, rgba(19, 19, 43, 0.95), rgba(30, 20, 60, 0.9)); border: 2px solid rgba(168, 85, 247, 0.5); border-radius: 24px; padding: 30px;">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 25px;">
            <h2 style="font-family: 'Exo 2', sans-serif; font-size: 1.8rem; background: linear-gradient(135deg, #a855f7, #ec4899); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0;">🔔 Хабарлар</h2>
